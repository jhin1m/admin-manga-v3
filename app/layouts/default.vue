    ...
    <UDashboardGroup>
      ...
      <UDashboardSidebar v-model:collapsed="collapsed" collapsible resizable>
        ...
        <div class="mt-6 flex-1">
          <UNavigationMenu :items="navigationItems" :collapsed="collapsed" orientation="vertical" :highlight="true" :ui="{
            link: 'py-3 px-4'
          }" />
        </div>
        ...
        <template #footer>
          <div class="p-4 border-t border-gray-200 dark:border-gray-800">
            ...
            <div class="flex items-center gap-2">
              <UColorModeButton />
              <UButton icon="i-lucide-log-out" color="neutral" variant="ghost" :label="collapsed ? '' : 'Đăng xuất'"
                @click="auth.logout()" />
            </div>
          </div>
        </template>
